...
from utils.tsne import load_tsne3d
...
class RetrievalTSNE(object):
    def __init__(self, root, label=None):
        paths, tsne3d, targets, classes = load_tsne3d(root)
        tsnes = [[] for i in classes]
        for tsne, target in zip(tsne3d, targets):
...
def main():
    paths, tsne3d, targets, classes = load_tsne3d('images/valid')
    rt = RetrievalTSNE('images/train')
    x = np.arange(1, 1 + len(tsne3d))
    colors = ['C1', 'C2']
    for i, label in enumerate(rt.classes):
        ...
        q = qs[:5]
        plt.subplot(221 + i)
        for t, target in zip(tsne3d, targets):
    ...
    # plt.suptitle(u'三维TSNE-欧式距离检索图片', **Kaiti)
    plt.savefig('TSNE/TSNE-欧式距离检索图片（三维）.png', dpi=300)
    plt.savefig('TSNE/TSNE-欧式距离检索图片（三维）.pdf', dpi=300)
